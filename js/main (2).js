...
const hostApp = window.location.hostname;;
            ...
            _mySky.getJSON(`${hostApp}/${dataKey}/${_id}`)
              .then((data) => {
                if (data.data == null) {
                  ...
                  //try {
                  //  _mySky.setJSON(`${hostApp}/${dataKey}/${_id}`,
                  //    JSON.parse(`{"uploads":[{"name":"${Date.now()}.svg","link":"${link}","date":${Date.now()}}]}`)
                  //  ).then(() => {
                  //    _dac.recordInteraction({ skylink: link, metadata: { action: 'new upload' } }).then(data => console.log(data)).catch(err => console.log(`error with CR DAC: ${err.message}`));
                  ...
                  //try {
                  //  _mySky.setJSON(`${hostApp}/${dataKey}/${_id}`, Json).then(test => {
                  //    _dac.recordInteraction({ skylink: link, metadata: { action: 'new upload' }}).then(data => console.log(data)).catch(err => console.log(`error with CR DAC: ${err.message}`));
                  //  });
